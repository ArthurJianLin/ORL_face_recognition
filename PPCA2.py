...
orlpath = "C:/Users/13199/Desktop/face/ORL_Faces"
...
def load_orl(k):
    ...
    '''
    train_face = np.zeros((40 * k, 112 * 92))   #训练集
    train_label = np.zeros(40 * k)  # [0,0,.....0](共40*k个0)
    test_face = np.zeros((40 * (10 - k), 112 * 92))   #测试集
    ...
    for i in range(40):  # 共有40个人
        ...
        for j in range(10):  # 每个人都有10张照片
            ...
            # 读取图片并进行矢量化
            img = img2vector(image)  #调用自定义函数并reshape
            if j < k:
                # 构成训练集
...
def PCA(data, r):
    ...
    return final_data, data_mean, V_r  #主成分变换矩阵
...
def face_rec():
    ...
    for r in range(10, 41, 10):  # 最多降到40维,即选取前40个主成分（因为当k=1时，只有40维)
        ...
        for k in range(1, 10):
            ...
            data_test_new = temp_face * V_r  # 得到测试脸在特征空间下的数据
            ...
            for i in range(num_test):
                ...
                sortedDistIndicies = sqDistances.argsort()  # 对排序向量从小到大排序，使用的是索引值,得到一个向量
            ...
            x_value.append(k)
            y_value.append(round(accuracy, 2))  #保留两位小数点
        ...
        if r == 10:
            y1_value = y_value
            plt.plot(x_value, y_value, marker="o", markerfacecolor="black")
        ...
        if r == 20:
            y2_value = y_value
            plt.plot(x_value, y2_value, marker="o", markerfacecolor="black")
            for a, b in zip(x_value, y_value):
                plt.text(a, b, (a, b), ha='center', va='bottom', fontsize=10)
        ...
        if r == 30:
            y3_value = y_value
            plt.plot(x_value, y3_value, marker="o", markerfacecolor="black")
            for a, b in zip(x_value, y_value):
                plt.text(a, b, (a, b), ha='center', va='bottom', fontsize=10)
        ...
        if r == 40:
            y4_value = y_value
            plt.plot(x_value, y4_value, marker="o", markerfacecolor="black")
            for a, b in zip(x_value, y_value):
                plt.text(a, b, (a, b), ha='center', va='bottom', fontsize=10)
    ...
    # 各维度下准确度比较
    L1, = plt.plot(x_value, y1_value, marker="o", markerfacecolor="blue")
    L2, = plt.plot(x_value, y2_value, marker="o", markerfacecolor="blue")
    L3, = plt.plot(x_value, y3_value, marker="o", markerfacecolor="blue")
    L4, = plt.plot(x_value, y4_value, marker="o", markerfacecolor="blue")
